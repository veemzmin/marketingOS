...
export default function CreateContentPage() {
  const handleSubmitSuccess = (contentId: string) => {
    redirect(`/dashboard/content/${contentId}/view`)
  }
...
export default async function EditContentPage({ params }: { params: { contentId: string } }) {
  ...
  if (content.status !== 'DRAFT') {
    redirect(`/dashboard/content/${params.contentId}/view`)
  }
            ...
            {filteredContents.length === 0 ? (
              ...
              filteredContents.map(content => (
                <tr key={content.id} className="hover:bg-gray-50">
                  ...
                  <td className="border p-3">
                    <Link
                      href={content.status === 'DRAFT' ? `/dashboard/content/${content.id}/edit` : `/dashboard/content/${content.id}/view`}
...
- Action links (Edit for DRAFT, View for others)
...
Action links route to edit (drafts) or view (other statuses).
...
9. Test edit vs view routing:
   - Content with DRAFT status → "Edit" link → opens editor
   - Content with SUBMITTED status → "View" link → (view page not built yet, may 404)
...
6. Status-based routing (edit for drafts, view for others)
